<template>
  ...
  <ContentWrap>
    ...
    >
      <el-table-column align="center" label="业务员" prop="salesmanName" min-width="100" />
      <el-table-column align="center" label="部门" prop="deptName" min-width="120" />
      <el-table-column align="center" label="手机号" prop="mobile" min-width="130" />
...
import { StatisticsApi, SalesmanStatisticsVO } from '@/api/erp/statistics'
import { formatNumber } from '@/utils'
import { formatDate } from '@/utils/formatTime'

defineOptions({ name: 'ErpSalesmanSalesStatistics' })

const loading = ref(true)
const list = ref<SalesmanStatisticsVO[]>([])
...
const getList = async () => {
  loading.value = true
  try {
    const data = await StatisticsApi.getSalesmanSalesStatistics(queryParams)
    list.value = data || []
  } finally {
    loading.value = false
  }
...
const resetQuery = () => {
  ...
  queryParams.salesmanName = undefined
